-- =====================================================
-- ADD SERVICE PROVIDERS JUNCTION TABLE (FIXED)
-- =====================================================
...
CREATE POLICY "Authorized users can insert service providers"
  ...
  WITH CHECK (
    EXISTS (
      SELECT 1 FROM public.shops WHERE id = service_providers.shop_id AND created_by = auth.uid()
    )
    OR
    EXISTS (
...
CREATE POLICY "Authorized users can update service providers"
  ...
  USING (
    EXISTS (
      SELECT 1 FROM public.shops WHERE id = shop_id AND created_by = auth.uid()
    )
    OR
    EXISTS (
      SELECT 1 FROM public.shop_staff
